...
*/
	...
	public function SaveData()
	{
		...
		foreach ($this->property_classes as $class):
		
			$result .= "+ Property:{$class}<br>\n";
			...
			while ($maxrows):
				...
				$query = "(MatrixModifiedDT={$this->startYear}-01-01T00:00:00+)";

				// run RETS search
				$result .= "   + Query: {$query}  Limit: {$limit}  Offset: {$offset}<br>\n";
				$search = $this->SearchQuery("Property", $class, $query, array('Limit' => $limit, 'Offset' => $offset, 'Format' => 'COMPACT-DECODED', 'Count' => 1));
				...
				if ($this->NumRows() > 0):
					...
					if ($offset == 1):
						...
						$fields_order = $this->SearchGetFields($search);
						fputcsv($fh, $fields_order);
							
					endif;

					// process results
					while ($record = $this->FetchRow($search)):
					...
					$offset = ($offset + $this->NumRows());

				endif;

				$maxrows = $this->IsMaxrowsReached();
				$result .= "    + Total found: {$this->TotalRecordsFound()}<br>\n";

				$this->FreeResult($search);
				
			endwhile;
	
			fclose($fh);
	
			$result .= "+ Process Finished<br>\n";
	...
	public function parseCSV()
	{
		...
		$result = "+ Parsing data from file {$this->dataFile}<br>\n";
		
		# Open data csv file
		$handle = fopen($this->dataFile, "r") or exit("+ Cannot open file {$this->dataFile}<br>\n");
		
		$recordCount = count(file($this->dataFile));
		
		$result .= "+ Record found: {$recordCount}<br>\n";
		...
		if ($handle):
			...
			while (($data = fgetcsv($handle, 10000, ",")) !== FALSE)
                $converted_data[] = array_combine($keys,$data);
		...
		if (!empty($converted_data)):
			...
			$db->query("TRUNCATE TABLE ".DB_TBLPRE.LISTINGS);
			
			$result .= "+ Truncated table ".DB_TBLPRE.LISTINGS."<br>\n";
			
			# Loop through data and save to database
			foreach ($converted_data as $key => $data):
					
				# Insert data into database
				
				$primary_id = $db->query_insert(LISTINGS,$data);
				
				$insert[] = $primary_id;
				
				if (UPDATE_LISTING):
				
					$property = $db->query_first("SELECT Matrix_Unique_ID,MatrixModifiedDT,PhotoModificationTimestamp FROM ".DB_TBLPRE.LISTING_DATES." WHERE Matrix_Unique_ID = ".$primary_id);
					
					if ($db->affected_rows > 0):
					
						$prop_date = strtotime($data['MatrixModifiedDT']);
						$prev_prop_date = strtotime($property['MatrixModifiedDT']);
						
						if ($prop_date > $prev_prop_date):
						
							$db->query_update(LISTINGS,array('update_listing' => '1'),"Matrix_Unique_ID = ".$data['Matrix_Unique_ID']);
							
						else:
							$db->query_update(LISTINGS,array('update_listing' => '0'),"Matrix_Unique_ID = ".$data['Matrix_Unique_ID']);
							
						endif;
						
						$photo_date = strtotime($data['PhotoModificationTimestamp']);
						$prev_photo_date = strtotime($property['PhotoModificationTimestamp']);
						
						if ($photo_date > $prev_photo_date):
						
							$db->query_update(LISTINGS,array('update_photo' => '1'),"Matrix_Unique_ID = ".$data['Matrix_Unique_ID']);
							
						else:
						
							$db->query_update(LISTINGS,array('update_photo' => '0'),"Matrix_Unique_ID = ".$data['Matrix_Unique_ID']);
							
						endif;
						
						$db->query_update(LISTING_DATES,array('MatrixModifiedDT' => $data['MatrixModifiedDT'],'PhotoModificationTimestamp' => $data['PhotoModificationTimestamp']),"Matrix_Unique_ID = ".$data['Matrix_Unique_ID']);
					
					else:
						
						$db->query_insert(LISTING_DATES,array('MatrixModifiedDT' => $data['MatrixModifiedDT'],'PhotoModificationTimestamp' => $data['PhotoModificationTimestamp']));
			...
			$result .= "+ Records inputed to database: {$count}<br>\n";
			$result .= "+ Data parsed and saved successfully to database.<br>\n";
			
			echo $result;
		
		else:
		
			exit("Data file was emtpy.<br>\n");
	...
	public function dirExists($dir_name = false, $path = './')
	{
		...
		if($tree && count($tree) > 0):
			...
			foreach($tree as $dir):
				...
				if($this->dirExists($dir_name, $dir.'/'))
					return true;
	...
	public function updatePropertiesPhoto()
	{
		
		global $db;
		
		echo "+ Getting property image folders.<br>\n";
		 
		# get all folders in specified directory
		$folders = glob($this->photoDir . "*", GLOB_ONLYDIR);
		...
		if (!empty($folders)):
			...
			foreach($folders as $folder):
				...
				if(is_dir($folder)):
					...
					if (is_numeric($currentFolder)):
						...
						$result = $db->query_first("SELECT * FROM ".DB_TBLPRE.LISTINGS." WHERE Matrix_Unique_ID = ".$currentFolder);
						...
						if ($db->affected_rows === 0):
							...
							rmdir($folder);
							$deleted[] = $currentFolder;
							echo "+ Deleted folder and contents: {$currentFolder}<br>\n";
						...
						rmdir($folder);
						$deleted[] = $currentFolder;
						echo "+ Deleted rogue folder and contents: {$currentFolder}<br>\n";
			...
			echo "+ Folders and content deleted: {$count}<br>\n";
			echo "+ Content successfully removed from server.<br>\n";
			
		else:
		
			echo "+ No content to remove.<br>\n";
			
		endif;
		
	}
	
	public function getValCount($v, $n)
	{
		
		return $v + ($n === $this->arrayVal);
		
	}
	
	public function getPropertiesPhoto()
	{
		...
		$result = $db->query("SELECT Matrix_Unique_ID,PhotoCount,update_photo FROM ".DB_TBLPRE.LISTINGS." WHERE PhotoCount != 0 ORDER BY Matrix_Unique_ID");
		
		echo "+ Total Properties to process: {$db->affected_rows}<br>\n";
		echo "+ Looping through properties for photos.<br>\n";
		
		while ($row = $db->fetch_array($result)):
			
			if ($row['update_photo'] == '1'):
			
				$return = $this->savePropertyPhotos($row['Matrix_Unique_ID']); # get images for property and save in database
				
				$array[] = $return;
				
				if ($return === false):
				
					echo "+ Error on pulling image and saving for ".$row['Matrix_Unique_ID'].".<br>\n";
					
				else:
				
					$db->query_update(LISTINGS,array('update_photo' => '0'),"Matrix_Unique_ID = ".$row['Matrix_Unique_ID']);
		...
		$this->arrayVal = false;
		$errors = array_reduce($array, array($this,'getValCount'), 0);
		
		$this->arrayVal = true;
		$success = array_reduce($array, array($this,'getValCount'), 0);
		
		$skipped = count($skipped);
		
		$folderCount = count(glob($this->photoDir . "*", GLOB_ONLYDIR));
			
		echo "+ Total folders present: {$folderCount}<br>\n";
		echo "+ Folders and content created: {$success}<br>\n";
		echo "+ Total skipped: {$skipped}.<br>\n";
		echo "+ Total errors: {$errors}.<br>\n";
	
	}
	
	
	public function savePropertyPhotos($sysid)
	{
	
		$photos = $this->GetObject('Property', 'Photo', $sysid, '*');
		...
		if ($photos):
			...
			foreach ($photos as $photo):
				...
				if ($photo['Success'] == true):
					...
					if (!$this->dirExists($sysid,$this->photoDir))
						mkdir($this->photoDir.$sysid ,0777);
					
					$file = $this->photoDir.$sysid.'/'.$sysid.'-'.trim($photo['Object-ID']).'.jpg';
					
					$data = $photo['Data'];
					
					if (file_exists($file)):
						
						if (strlen($data) !== filesize($file)):
		...
		return (!empty($error) && array_sum($error) > 0) ? false : true;
	...
	public function Connect($login_url, $username, $password, $ua_pwd = "") {
		...
		if (!isset($this->last_request['ReplyCode']) || $this->last_request['ReplyCode'] == 0) {
			return true;
		}
		else {
